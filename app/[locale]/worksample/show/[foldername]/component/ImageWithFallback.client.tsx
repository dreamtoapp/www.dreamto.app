...
export default function ImageWithFallback({ image }: { image: ImageType }) {
  ...
  return (
    <div className="relative flex items-center justify-center p-4">
      <div className="relative w-full pb-[100%] rounded-2xl overflow-hidden shadow-lg group bg-muted">
        {hasError ? (
          <Image
      ...
      {hasError && (
        <button
          ...
          className="absolute bottom-4 left-1/2 transform -translate-x-1/2 px-4 py-2 bg-primary text-primary-foreground font-semibold rounded-lg shadow-md hover:bg-primary/90 transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2"
        >
          {t("reloadImage")}
        </button>
      )}
